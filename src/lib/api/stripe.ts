...
export const handleWebhook = async (event: Stripe.Event) => {
  serverLogger.info({
    type: "STRIPE", msg: "Handling webhook", details: {
      event: {
        type: event.type,
        id: event.id,
      }
...
async function fulfillOrder(session: Stripe.Checkout.Session) {
  ...
  serverLogger.info({
    type: "STRIPE", msg: "Fulfilling order", details: {
      session: {
  ...
  if (!session.client_reference_id) {
    serverLogger.error({ type: "STRIPE", msg: "Fulfilling order error", details: { sessionId: session.id, error: "Order ID not found in Stripe webhook event" } });
  ...
  if (IS_PRODUCTION) {
    ...
    try {
      ...
      serverLogger.error({ type: "STRIPE/AUTH", msg: "Error sending verification code during order fulfillment. User will need to re-send the code.", details: { userId, email, error } });
    }
  }
  ...
  await db.transaction(async (tx) => {
    ...
    await tx
      .update(orders)
      .set({ status: "paid", userId })
      .where(eq(orders.id, session.client_reference_id!));
  });

  serverLogger.info({ type: "STRIPE/AUTH", msg: "User updated with allowed sites. User added to order.", details: { userId, allowedSites, orderId: session.client_reference_id } });
