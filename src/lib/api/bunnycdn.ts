...
import { BUNNYCDN_ZONE_API_KEY, BUNNYCDN_API_URL, BUNNYCDN_STORAGE_URL, BUNNYCDN_ACCOUNT_API_KEY } from "../constants/server/bunnycdn.server";
...
export const uploadFile = async ({ path, file, purge }: { path: string, file: Buffer, purge?: boolean }) => {
  ...
  if (purge) {
    ...
    try {
      ...
      if (!response.ok) {
        serverLogger.error({ type: "API", msg: "Error purging file. Ignoring.", details: { path, purgeFileUrl, status: response.status, response: await response.json() } });
